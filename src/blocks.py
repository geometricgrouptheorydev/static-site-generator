...
#we split markdown into blocks that will be parsed to HTML independently
...
def block_to_block_type(block):
    ...
    elif False not in [lines[i].startswith(f"{i+1}.") for i in range(0,len(lines))]: #checks if each line starts with 1., 2., 3., etc.
